...
SFQuest_PlayerHandler = SFQuest_PlayerHandler or {}
SFQuest_PlayerHandler.QuestDb = {}

local function initGlobalModData(isNewGame)
    SFQuest_PlayerHandler.QuestDb = ModData.getOrCreate("missionProgress")
    if isNewGame then 
        print("- New Game Initialized!") 
    else 
        print("- Existing Game Initialized!") 
    end
end
...
function SFQuest_PlayerHandler.StartPlayerFix(int, player)
	--  se QusetDb è una tabella vuota allora il giocatore ha perso i dati lato client (global_mod_data.bin corrotto o perso, cosa quasi impossibile, di solito succede quando formatti che lo perdi, o quando cambi IP poiché cambia la cartella)
	-- se QuestDb non è maggiore di 0 allora il giocatore è nuovo
	if next(QuestDb) == nil then
		-- controlliamo prima se esiste un backup .txt nel server da ripristinare
		if isClient() then
            print("zSOUL QUEST SYSTEM - Richiesta dei dati di backup delle missioni [pre init].")
            local id = player:getUsername()
            sendClientCommand(player, 'SFQuest', 'sendData', {id = id})
        else
			-- se siamo in single player e non c'è il backup, AMEN, si reinizializza con i dati di SFQuest_PlayerHandler.startingPlayerStats
			QuestDb["missionProgress"] = SFQuest_PlayerHandler.startingPlayerStats;
			-- ECCECCECCECC
			
			ModData.add("missionProgress", QuestDb)
		end
			
			
	else 
		print("zSOUL QUEST SYSTEM - dati recuperati dal file locale global_mod_data.bin")
	end
end
Events.OnInitGlobalModData.Add(initGlobalModData)
...
Events.OnServerCommand.Add(function(module, command, player, args)
    if module == 'SFQuest' and command == 'sendData' then
        local id = args.id
        local filepath = "/Backup/SFQuest_" .. id .. ".txt"
        local filereader = getFileReader(filepath, false)
        local backupData = {}
        if filereader then
            local line = filereader:readLine()
            while line ~= nil do
                table.insert(backupData, line)
                line = filereader:readLine()
            end
            filereader:close()
            -- Potrebbe essere necessario convertire le linee in una struttura di tabella appropriata
        end
        -- Invia i dati di backup al client
        sendClientCommand(player, 'SFQuest', 'sendData', {data = backupData})
    end
end)
